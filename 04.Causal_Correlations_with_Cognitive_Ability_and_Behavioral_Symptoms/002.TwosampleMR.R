...
mr_pip <- function(exposure, outcome, iv_plink, exp_trait, out_trait, pvalue, respath,samplesize_exp,samplesize_out){
  ...
  exp_dat <- format_data(    ####该函数用于创建单变量MR所需要的暴露的输入文件，注意这里的暴露并未与clump过后的SNP取交集
    ...
    #eaf_col = "eaf", #函数默认识别名为eaf的列，因此不用额外设置该参数
    beta_col = "beta",
    se_col = "se",
    pval_col = "p",
  ...
  if (nrow(merge) != 0) {
    ...
    ####calculate relative r square manually when exposure/outcome is binary trait to make directionality test more accurate
    #MDD is binary trait
    #dat$r.outcome <- get_r_from_lor(lor = dat$beta.outcome,af = dat$eaf.outcome,ncase = 170756,ncontrol = 329443,prevalence = 0.052)###查询MDD在欧洲人群当中的一年流行率
...
### iv_path
clump_F <- 'E:/Projects_documents/data/GWAS_sumstat20241101/anxiety_MultiAllergicDiseases_project_GWASsumstats/clump_result/02.p5e6/' #clump结果目录

#exp_anxiety_path <- '/public/jiangjw/02.allergy_anxiety/data/Anxiety/'
...
out_path <- 'E:/Projects_documents/data/GWAS_sumstat20241101/EUR/'

result_path <- 'E:/Projects_documents/05.allergic_diseases_mental_disorders/01.anxiety_ADs/01.MR/02.5e-6/5.0.Behavior_cognition_to_traits/' #结果存放目录
...
for(i in 1:length(exp_trait1))
{
  for(j in 1:length(out_trait))
  {
    tmp_exp <- paste0(exp_trait1_path,exp_trait1[i],".clean.txt")
    tmp_out <- paste0(out_path,out_trait[j],".clean.txt")
    tmp_iv <- paste0(clump_F,exp_trait1[i],".LD_result.clumped")
    mr_pip(exposure = tmp_exp,    
           outcome = tmp_out,    
           iv_plink = tmp_iv,     ###clump结果
           exp_trait = exp_trait1[i],
...
##### attention to the existence of eaf #####
for(k in 1:length(exp_trait2))
{
  for(l in 1:length(out_trait))
  {
    tmp_exp <- paste0(exp_trait2_path,exp_trait2[k],".clean.txt")
    tmp_out <- paste0(out_path,out_trait[l],".clean.txt")
    tmp_iv <- paste0(clump_F,exp_trait2[k],".LD_result.clumped")
    mr_pip(exposure = tmp_exp,    
           outcome = tmp_out,    
           iv_plink = tmp_iv,     ###clump结果
           exp_trait = exp_trait2[k],
           out_trait = out_trait[l],
           pvalue = 1,
           respath = result_path,
          )
