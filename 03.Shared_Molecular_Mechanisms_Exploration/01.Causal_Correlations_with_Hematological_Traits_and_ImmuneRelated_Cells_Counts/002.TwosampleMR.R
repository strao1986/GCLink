...
#########################################################################
#exp_trait <- c("anxiety2018","anxiety2021","AR_Diagnose","AR_SelfReport","Asthma2018","Asthma2020","ADE2015","ADE2021","IL-1a","IL-1b","IL-4","IL-6")
##########################################################################
out_traits_anxiety <- c("anxiety2018","anxiety2021")
out_traits_allergy <- c("AR_Diagnose","AR_SelfReport","Asthma2018","Asthma2020","ADE2015","ADE2021")  #暴露因素
exp_traits_IL <- c("BASO","CD19","CD3","CD4CD8ratio","CD4","CD56","CD8","EOS","HB","HT","LYMPH","MCHC","MCH","MCV","MONO","NEUT","PLT","RBC","WBC")  #结局变量
...
### iv_path
clump_F <- '/public/jiangjw/02.allergy_anxiety/data/clump_result/02.p5e6/' #clump结果目录

out_anxiety_path <- '/public/jiangjw/02.allergy_anxiety/data/Anxiety/'
out_allergy_path <- '/public/jiangjw/02.allergy_anxiety/data/Allergic_diseases/'
exp_data_path <- '/public/jiangjw/02.allergy_anxiety/data/molecules/clean_data_GRCh37/19molecules/'

result_path <- '/public/jiangjw/02.allergy_anxiety/01.MR/004.19molecules_to_anxiety_allergy/02.p5e6/' #结果存放目录

### TSMR

for(i in 1:length(exp_traits_IL))
{
	for(j in 1:length(out_traits_anxiety))
	{
		        tmp_exp <- paste0(exp_data_path,exp_traits_IL[i],".clean.txt")
        		tmp_out <- paste0(out_anxiety_path,out_traits_anxiety[j],".clean.txt")
			tmp_iv <- paste0(clump_F,exp_traits_IL[i],".LD_result.clumped")
			mr_pip(exposure = tmp_exp,    
         			outcome = tmp_out,  
         			iv_plink = tmp_iv,     ###clump结果
         			exp_trait = exp_traits_IL[i],
         			out_trait = out_traits_anxiety[j],
         			pvalue = 1,
         			respath = result_path,
...
for(k in 1:length(exp_traits_IL))
{
        for(l in 1:length(out_traits_allergy))
        {
                        tmp_exp <- paste0(exp_data_path,exp_traits_IL[k],".clean.txt")
                        tmp_out <- paste0(out_allergy_path,out_traits_allergy[l],".clean.txt")
                        tmp_iv <- paste0(clump_F,exp_traits_IL[k],".LD_result.clumped")
                        mr_pip(exposure = tmp_exp,    
                                outcome = tmp_out,    
                                iv_plink = tmp_iv,     ###clump结果
                                exp_trait = exp_traits_IL[k],
                                out_trait = out_traits_allergy[l],
                                pvalue = 1,
                                respath = result_path,
                                #samplesize_exp = tmp_samplesize_exp,
                                #samplesize_out = tmp_samplesize_out
