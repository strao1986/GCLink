...
outtran <- c("anxiety2018","anxiety2021","AR_Diagnose","AR_SelfReport","Asthma2018","Asthma2020","ADE2015","ADE2021") 
exptran <- c("BASO","CD19","CD3","CD4CD8ratio","CD4","CD56","CD8","EOS","HB","HT","LYMPH","MCHC","MCH","MCV","MONO","NEUT","PLT","RBC","WBC")

info_clumped <- data.frame()
datapath <- "/public/jiangjw/02.allergy_anxiety/01.MR/004.19molecules_to_anxiety_allergy/02.p5e6/"
clump_dir <- "/public/jiangjw/02.allergy_anxiety/data/clump_result/02.p5e6/"

outtran_tmp <- outtran[1]  #only focus on the number of exposure clumped SNPs 
for(i in 1:length(exptran))
{
  clumped_snp <- fread(paste0(datapath,exptran[i],"-",outtran_tmp,"/clump^exp_clean.csv"))
...
info_full <- select(info_full,exposure,outcome,clumped_snp,No..of.SNPs.in.MRA,MR.pvalue,Adjusted_pval,beta_se,Method,heterogeneity,pleiotropy,directionality,everything())
